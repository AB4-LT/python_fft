...
def calc_python_velocity(freq_array, fft_array):
    ...
    for i in range(CALC_START_POS, len(freq_array)):
        if (freq_array[i] > 9.5) & (freq_array[i] < 1000) & (fft_array[i] > DEAD_ZONE) :
            velocity_fft[i] =  (fft_array[i]/pow(2, 0.5)) * G_MM_S2 * MM_IN_METER / (2*pi * freq_array[i]) #перевод из амплитуды в дискретах ускорения в СКЗ виброскорости на этой частоте
            count += 1
            velocity += pow( velocity_fft[i], 2)
...
if 0 :
    ...
    input_points = []
    input_signal_hz = 0.09
    file_descr = " model " + str(input_signal_hz) +" Hz [" + str(USE_POINTS) + " points " + str(FD) + " Hz]"
    for i in range(USE_POINTS):
        model_signal = 1/G_SCALE_FACTOR*sin(input_signal_hz*i/FD*2*pi+pi/2)
        #model_signal += 1/G_SCALE_FACTOR*sin(900*i/FD*2*pi)
